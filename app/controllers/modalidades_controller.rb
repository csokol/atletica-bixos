class ModalidadesController < ApplicationController
  ...
  # GET /modalidades.xml
  def index
    @modalidades = Modalidade.find(:all)
  ...
  def create
    ...
    respond_to do |format|
      if @modalidade.save
        flash[:notice] = 'Modalidade was successfully created.'
        format.html { redirect_to(@modalidade) }
  ...
  def update
    ...
    respond_to do |format|
      if @modalidade.update_attributes(params[:modalidade])
        flash[:notice] = 'Modalidade was successfully updated.'
        format.html { redirect_to(@modalidade) }
        format.xml  { head :ok }
      else
